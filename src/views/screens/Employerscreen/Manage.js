...
return(
   ...
   <View style={{borderWidth: 2, borderColor: '#e8e8e8', margin: 5, borderRadius: 10, padding: 5}}>
     ...
     { arr.company[0]   && <Text style={Universalstyles.text}><Icon name='warehouse' style={{fontSize: 20, color: 'black', marginRight: 10}}/> {arr.company[0].comp_name}</Text>}
      ...
      <Text style={Universalstyles.text}><Icon name='map-marker' style={{fontSize: 20, color: 'red', marginRight: 10}}/> {arr.address[0].street}, {arr.address[0].city}, {arr.address[0].province}, {arr.address[0].zipcode}</Text>
    ...
    <Text style={Universalstyles.text}><Icon name='currency-php' style={{fontSize: 20, color: 'red', }}/> {arr.salary} {arr.rate} </Text>
      ...
      {/* {console.log(arr.applies[0].user[0].id)} */}
...
<View style={{height: 'auto', borderWidth: 2, borderColor: '#e8e8e8', borderRadius: 0, margin: 0}}>
   ...
   {label.apply__user__userdetails__profile ? <Image source={{uri:label.apply__user__userdetails__profile}} style={{
      width: 70,
      height: 70,
      marginRight: 7,
      borderRadius: 35,
    ...
    <Text style={{ opacity: .5, fontSize: 12}}>{label.apply__user__address__street} {label.apply__user__address__city} {label.apply__user__address__province} {label.apply__user__address__zipcode} <Text style={{color: 'green', textTransform: 'capitalize'}}></Text> </Text>
    <Text style={{ opacity: .5}}>{label.apply__user__userdetails__birthday}<Text style={{color: 'green', textTransform: 'capitalize'}}></Text> </Text>
        </View>
  ...
  [
    {
      text: "Yes",
      onPress: () => {
        axiosRequest.post('auth/applicant/view/app/atay',JSON.stringify({status: "Decline",apply:label.apply__id}),{
          headers: {
            'Content-Type': 'application/json'
          }
        }).then((response) => {
          Alert.alert("Decline","Application Successfully Decline",
          [
      {
        text: "Okay!",
    ...
    { 
      text: "No", onPress: () => console.log(JSON.stringify({status: "Decline",apply:label.apply__id}), {
        headers: {
          'Content-Type': 'application/json'
        }
      })
  ...
  [
    {
      ...
      onPress: () => {
        axiosRequest.post('auth/applicant/view/app/atay',JSON.stringify({apply:label.apply__id,status:"Approved"}), {
          ...
          Alert.alert("Approved","Go to Applicant to Manage Approved Applicants",
            [
        {
          text: "Okay!",
  ...
  <MenuOptions optionsContainerStyle={{ marginLeft:-15,width:300,position:"relative",top:5 }}>
    ...
    <Text> No Applicant To display! </Text>
  </View>
}
</SafeAreaView>
);
...
navigation.setOptions({
 ...
 navigation.addListener('focus', () => {
  ...
  axiosRequest.get(`auth/post/manage/${user.id}/${post}/`)  
  .then((response) => {
  ...
  axiosRequest.get(`auth/user/applied/${post}/`)  
      .then((response) => {
        // console.log(response.data)
  ...
  // console.log(manage)
    ...
    const renderScene = ({ route }) => {
        switch (route.key) {
          case 'first':
            return <FirstRoute navigation={navigation} arr = {manage[0]}/>;
          case 'second':

            return <SecondRoute navigation={navigation}  profile = {Profile} arr = {applied.posts}  />;
